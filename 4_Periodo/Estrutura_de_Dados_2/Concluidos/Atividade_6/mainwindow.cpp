...
void MainWindow::on_BuscaSimples_clicked()
{
    try {
        ...
        if(obj->BuscarNum(ui->lineEditValor->text().toInt()) == -1){
            ui->textEditNExecucoesSequencialS->setText(QString::number(obj->getExecucoes()));
            ui->textEditResultadoSimples->clear();
            throw QString ("Numero nao encontrado");
        }
        else{
            ui->textEditResultadoSimples->setText("O numero que deseja esta na posicao: [" +
...
void MainWindow::on_pushButtonBuscaMelhorada_clicked()
{
    try {
        ...
        if(obj->buscarNumMelhorado(ui->lineEditValorMelhorada->text().toInt()) == -1){
            ui->textEditNExecucoesSequencialM->setText(QString::number(obj->getExecucoes()));
...
void MainWindow::on_pushButtonBuscaBinaria_clicked()
{
    try {
        ...
        if(obj->buscaBinaria(ui->lineEditValorBi->text().toInt()) == -1){
            ui->textEditNExecucoesBi->setText(QString::number(obj->getExecucoes()));
