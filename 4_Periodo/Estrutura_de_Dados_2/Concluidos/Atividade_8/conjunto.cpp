...
int Conjunto::getExecucoesBS() const
{
    return execucoesBS;
}
...
double Conjunto::TempoBubbleSort()
{
    // Registre o tempo de início para o insertionSort
    auto start_timeIS = std::chrono::high_resolution_clock::now();
...
QString Conjunto::obterVetorBubbleSort()
{
    ...
    QString resultado = "|";
    for (int pos = 0; pos < tamanho; pos++) {
        resultado += QString::number(arrayBubbleSort[pos]);
        if (pos < tamanho - 1)
            resultado += "|->|";
    }
    resultado += "|";
